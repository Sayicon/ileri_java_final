...
**Sorumlu: Kerem** · **Süre: 2 gün**

#### A — Testler (önce commit'le)
- [ ] `ApiResponse<T>` success/error/serialization testleri
- [ ] `PagedResult<T>` pagination hesaplamaları
- [ ] `Validator<T>` ve `ValidationResult` testleri
- [ ] RFC 7807 `ProblemDetail` alan adları
- [ ] `DomainException` alt sınıfları → doğru HTTP status
- [ ] Wildcards: `? extends T` ve `? super T` en az birer örnek
- [ ] **Commit:** `test(faz1): generic API response, paged result, validator, problem detail`

#### B — Uygulama
- [ ] `ApiResponse<T>`, `PagedResult<T>`, `ProblemDetail`
- [ ] `Validator<T>` interface + `ValidationResult` record
- [ ] `Repository<T, ID>` interface
- [ ] Exception hiyerarşisi: `DomainException` → `NotFoundException`, `ConflictException`, `ValidationException`, `UnauthorizedException`, `ForbiddenException`, `RateLimitException`
- [ ] `CollectionOps.java`: wildcards demo
- [ ] `mvn -pl shared test` → yeşil · `test-logs/faz-1-green.txt`
- [ ] **AGENTS.md güncelle**
